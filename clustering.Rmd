...
full_data <- left_join(prescription_data, overdose_data, by = c("year", "State")) %>% 
  ...
  janitor::clean_names() %>% 
  mutate(pres_std = scale(prescription_rate)
         , age_adj_std = scale(age_adjusted_rate)) %>%
  filter(year == 2018) %>% 
  select(state, pres_std, age_adj_std, prescription_rate
         , age_adjusted_rate)
...
ggplot(data = full_data, aes(x = pres_std, y = age_adj_std)) +
